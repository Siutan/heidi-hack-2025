...
contextBridge.exposeInMainWorld('electron', {
  ...
  wakeWord: {
    ...
    onStatusChange: (callback: (status: string) => void) => {
      ...
      return () => ipcRenderer.removeListener('wake-word-status-change', listener);
    },
    
    onWakeDetected: (callback: (data: { transcript: string; confidence: number }) => void) => {
      const listener = (_event: Electron.IpcRendererEvent, data: { transcript: string; confidence: number }) => callback(data);
